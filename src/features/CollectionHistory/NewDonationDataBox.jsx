...
function NewDonationDataBox({ collection }) {
  const {
    donation_id,
    created_at,
    food_name: item,
  ...
  return (
    <StyledDonationDataBox>
      ...
      <Footer>
        <p>
          {status === "rejected"
            ? `Rejected on ${formatDate(new Date(created_at))}`
            : status === "received"
            ? `Received on ${formatDate(new Date(created_at))}`
            : null}
        </p>
      </Footer>
    </StyledDonationDataBox>
...
export default NewDonationDataBox;
